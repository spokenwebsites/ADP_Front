    ...
    """
        ...
        for i in range(len(deserialised_json)):
            if "Dates" in deserialised_json[i]:
                if "date" in deserialised_json[i]["Dates"][0]:
                    date = deserialised_json[i]["Dates"][0]["date"]
                    Count_events.append(date)
                    if date not in dateToIds:
                        dateToIds[date] = []
                    dateToIds[date].append(deserialised_json[i]['swallow_id'])
        ...
        for vdate in Events_withcount:
            try:
                ...
                error_swallows[vdate] = dateToIds[vdate]
