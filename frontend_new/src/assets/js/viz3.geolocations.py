...
def createcsvwithcoordinates():
  ...
  with open('frontend_new/src/assets/js/Draft.csv', 'w', encoding='UTF8', newline='') as f:
...
#
def getcitiescount():
  df=pd.read_csv("frontend_new/src/assets/js/Draft.csv")
  ...
  arr_finaldata=[]
  with open('frontend_new/src/assets/js/Draft.csv', 'r', newline='') as file:
      header = ['city','homelat','homelon','homecontinent','state','n']
      with open('frontend_new/src/assets/js/final1.csv', 'w', encoding='UTF8', newline='') as f:
          writer = csv.writer(f)
          writer.writerow(header)
